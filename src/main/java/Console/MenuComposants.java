...
public class MenuComposants {
    ...
    public void Materiau(Project project) {
        ...
        do {
            ...
            double MainOeuvreQuantity = validation.getValidDoubleInput("Entrez la quantité de ce MainOeuvre (en m²) : ");
            double MainOeuvreUnitCost = validation.getValidDoubleInput("Entrez le coût unitaire de ce MainOeuvre ($/m²) : ");
            scanner.nextLine(); 
    
            String MainOeuvreType = validation.getValidStringInput("Entrez le type de main-d'oeuvre (e.g., Ouvrier de base, Spécialiste) : ");
            double tauxHoraire = validation.getValidDoubleInput("Entrez le taux horaire de cette main-d'oeuvre ($/h) : ");
            double heuresTravail = validation.getValidDoubleInput("Entrez le nombre d'heures travaillées : ");
            double MainOeuvreProductivityFactor = validation.getValidDoubleInput("Entrez le facteur de productivité (1.0 = standard, > 1.0 = haute productivité) : ");
    ...
    public void calculerCoutDetaille(Map<Integer, Materiaux> materiauxMap,
        ...
        LocalDate dateValidite =validation.getValidDate("Entrez la date de validité du devis (format : jj/mm/aaaa) : ");
